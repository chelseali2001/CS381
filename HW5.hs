...
checkExpr [] (Ref i)        = False
checkExpr x  (Ref i)        = elem i (x)
...
checkCmd i l (Pen m)       = if m == Up || m == Down then True else False
checkCmd i l (Move x y)    = checkExpr l x && checkExpr l y
checkCmd i l (Call m a)    = if get m i == Just (length a) && all (== True) (exprHelp l a) then True else False
checkCmd i l (For v x y b) = checkExpr (l ++ [v]) x && checkExpr (l ++ [v]) y && checkBlock i (l ++ [v]) b

exprHelp :: [Var] -> [Expr] -> [Bool]
exprHelp l []     = []
exprHelp l (x:xs) = [checkExpr l x] ++ exprHelp l xs
...
checkBlock _ _ [] = True
checkBlock i l (x:xs) = checkCmd i l x && checkBlock i l xs
...
checkDef l (Define m p b) = checkBlock l p b
...
cmd defs env state@(pen,pos) c = case c of
    ...
    Move xExp yExp -> if (pen == Up) 
                      then ((Up, (expr env xExp, expr env yExp)), [])
                      else ((Down, (expr env xExp, expr env yExp)), [((pos), (expr env xExp, expr env yExp))])

    Call macro args ->
      case getOrFail macro defs of
        ([], []) -> ((pen, pos), [])
        (a, b) ->
          case (map (expr env) args) of
            c -> block defs (setAll a c []) state b
    ...
    For v fromExp toExp body ->
      ...
      let from = expr env fromExp
          ...
          loopStep (s, ls) i =
            let (s', ls') = helper0 defs (env++[("i", i)]) s body
            in (s', ls ++ ls')

      in foldl loopStep (state, []) ixs

helper0 :: Macros -> Env -> State -> [Cmd] -> (State, [Line])
helper0 _ _ s [] = (s, [])
helper0 defs env s [x] = cmd defs env s x
